  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center w-full h-full overflow-hidden backdrop-blur-sm">
        ...
        <button className="absolute p-2 rounded-sm top-5 right-5 bg-midnight hover:bg-gray-500">
            <img src="public/logos/close.svg" alt="" className="w-6 h-6" />
        </button>
        ...
        <div className="p-5">
            ...
            {subDescription.map((subDesc, index)=>(
                <p className="font-normal mb-3 text-neutral-400">{subDesc}</p>
            ))}
            <div className="flex items-center justify-between mt-4">
                <div>
                    {tags.map((tag)=>(
                        <img key={tag.id} src={tag.path} alt={tag.name} className="rounded-lg size-10 hover-animation">{tag.name}</img>
                    ))}
                </div>
